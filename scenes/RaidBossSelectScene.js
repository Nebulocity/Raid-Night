...
const BOSS_SHORT_NAMES = {
  ...
  high_chief_bonkgar:                'High Chief Bonkgar',
};
...
export default class RaidBossSelectScene extends Phaser.Scene {
  ...
  _drawBossButton(x, y, boss, raid, saveData, unlocked, defeated, buttonSize) {
    ...
    if (defeated) {
      nameText.setText('Defeated').setColor('#aa4444');

      const gfx = this.add.graphics().setDepth(DEPTH_NAMETEXT);
      const half = buttonSize / 2;
      gfx.lineStyle(6, 0xcc0000, 0.9);
      gfx.beginPath();
